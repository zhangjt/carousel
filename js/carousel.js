...
(function() {
    ...
    Carousel.prototype = {
        ...
        touchRotate: function() {
            // var elemTouchTarget=document.getElementsByClassName('poster-main')[0]
            var self = this;
            //事件类型
            EVENT_TYPE = ('ontouchend' in document) ? 'touchstart' : 'click';
            var startPos;
            this.poster[0].addEventListener('touchstart', function(event) {
            ...
            this.poster[0].addEventListener('touchend', function(event) {
            	event.stopPropagation(); //取消事件冒泡;
                var target = event.target;
                ...
                deltaX = endPos.x - startPos.x; //delta增量
